...
import (
	...
	platformMetrics "github.com/Artyom099/factory/platform/pkg/metrics"
)
...
const (
	...
	metricRequestDuration     = serviceName + "_http_request_duration_seconds"
	metricErrorsTotal         = serviceName + "_errors_total"
)

var (
	// Meter - это интерфейс для создания и регистрации метрик в OpenTelemetry.
	// Он действует как фабрика для создания различных инструментов измерения, таких как:
	// - Счетчики (Counter): накапливают значение, которое может только увеличиваться (например, число запросов)
	// - Гистограммы (Histogram): отслеживают распределение значений (например, время ответа)
	// - Измерители (Gauge): могут увеличиваться и уменьшаться (например, использование памяти)
	// - Измерители up/down (UpDownCounter): могут увеличиваться и уменьшаться дискретно
	//
	// Meter предоставляет методы для регистрации этих инструментов с определенным именем
	// и предоставляет дополнительные метаданные, такие как описание и единицы измерения.
	// Это ключевой компонент телеметрии, т.к. через него происходит вся работа с метриками.
	meter metric.Meter
...
// Эта функция должна быть вызвана перед использованием метрик
// Провайдер метрик (MeterProvider) - это основной компонент, который регистрирует
// и управляет всеми метриками. Он создает "meter" для каждого сервиса/компонента
// и соединяет их с экспортерами данных.
// Экспортер отправляет метрики в систему хранения через OpenTelemetry Collector.
// Принимает контекст снаружи для корректной работы с отменой и трассировкой
func InitMetrics(ctx context.Context, cfg config.MetricServerConfig) error {
	// Если метрики уже инициализированы, ничего не делаем
	if meter != nil {
		return nil
	}

	// Инициализируем провайдер метрик через общую библиотеку
	err := platformMetrics.InitProvider(ctx, cfg)
	if err != nil {
		return err
	}

	// Получаем meter из провайдера для нашего сервиса
...
func CountOrderCreated(ctx context.Context, status string) {
	// Если метрики не инициализированы, ничего не делаем
	if meter == nil {
		return
	}

	// Увеличиваем счетчик заказов по статусам
	orderCounter.Add(
		ctx,
		1,
...
// ShutdownMetrics закрывает провайдер метрик и экспортер
